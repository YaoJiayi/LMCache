...
for idx, chunk_ids in enumerate(doc_chunk_ids):
    if idx == 0:
        ...
        doc_chunk_ids_with_sep.append([422]+chunk_ids)
input_ids = []
...
input_ids_new = tokenizer.encode(input_prompt)

split_at = 422
token_chunks = [list(g) for k, g in groupby(input_ids_new, lambda x: x != split_at) if k]
pdb.set_trace()
